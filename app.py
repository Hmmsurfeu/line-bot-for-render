...
def handle_message(event):
    with ApiClient(configuration) as api_client:
        # 當使用者傳入文字訊息時
        print("dddddddddddddd",{event.message.text})
        line_bot_api = MessagingApi(api_client)
        user_msg = event.message.text
        # bot_msg = TextMessage(text=f"What ydd阿斯顿发生发射点发生ou said is: {user_msg}")

        if user_msg in faq:
            bot_msg= faq[user_msg]
        elif user_msg.lower() in ["menu","home"]:
            bot_msg=menu
        elif user_msg in table:
            buy=table[user_msg]["buy"]
            sell=table[user_msg]["sell"]
            bot_msg=TextMessage(text=f"{user_msg} is {buy} or {sell}")
        ...
        # if user_msg=="phone":
        #     bot_msg=TextMessage(text="383838383838")
